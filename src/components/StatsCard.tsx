...
export default function StatsCard({ title, value, icon: Icon, color, delay = 0, onClick }: StatsCardProps) {
  ...
  const getGlowColor = (colorClass: string) => {
    ...
    if (colorClass.includes('yellow')) return 'rgba(234, 179, 8, 0.15)'
    if (colorClass.includes('red')) return 'rgba(239, 68, 68, 0.15)'
  ...
  return (
    <motion.div
      ...
      whileHover={{
        y: -4,
        scale: 1.03,
        boxShadow: `0 20px 40px ${getGlowColor(color)}, 0 0 20px ${getGlowColor(color)}`
      }}
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 -translate-x-full group-hover:translate-x-full transform-gpu"
      ...
      <div className="relative z-10 flex items-center justify-between">
        ...
        <motion.div
          ...
          style={{
            boxShadow: `0 8px 16px ${getGlowColor(color)}`
          }}
        >
